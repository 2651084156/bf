...
def callback(ch, method, properties, body): #返回的是字节
    ...
    print('开始处理：'+ccc)
    a123 =analyze(link=ccc,get_rule=[{'get_link_f': ''} ,{'code_get_m':''} ,{'xpath_get':['main','/html/body/div[2]/div[1]/div/ul/*//a/@href','back']},{'for_list':['back','abd',[ {'add_list':['aaaee','abd']}] ]},{'return_num':'aaaee'}],uuid=114514)
    a123.run()
    ccc =a123.rep_back
    for i in ccc:
        ...
        send_get_list_work(body=aa, queue='senc', exchange='senc')  # 最好这里也输入字节
    # print(properties)
    # print(method)
