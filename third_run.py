...
def callback(ch, method, properties, body): #返回的是字节
    ...
    print('开始处理：' + ccc)
    a123 =analyze(link=ccc,get_rule=[{'get_link_f': ''} ,{'code_get_m':''} ,{'xpath_get':['main','/html/body/div[2]/div/div[3]//p//text()','back']},{'for_list':['back','abd',[ {'add_list':['aaaee','abd']}] ]},{'return_num':'aaaee'}],uuid=114514)
    a123.run()
    ccc = a123.rep_back
    asdx =''
    for i in ccc:
    ...
    print(asdx)
    aads =asdx.encode('utf-8')

    send_get_list_work(body=aads, queue='back', exchange='back')  # 最好这里也输入字节
